...
use crate::{
    DirectDigitRangeProofBackend, DirectRelationRangePreparationInput,
    DirectRelationRangeProofBackend, RecursiveWitnessFlat,
};
    ...
    Cfg: CommitmentConfig<Field = F, ExtField = E>,
{
    ...
    let prepare_stage2 = || {
        ...
        let preparation = stack.opening().backend().prepare_direct_relation_range(
            ...
            DirectRelationRangePreparationInput::new(
    ...
    let (stage1_result, stage2_static) =
        if stack.opening().backend().execution_domain() == ComputeExecutionDomain::Accelerator {
