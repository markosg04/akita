...
impl<E: Field + Ring + Fold + Unreduced> DirectRelationRangeProofState<E> {
    ...
    pub fn current_coefficient_count(&self) -> Result<usize, AkitaError> {
        Ok(self.common_alpha_factor()?.len())
    }

    pub fn current_lane_capacity(&self) -> Result<usize, AkitaError> {
        Ok(self.weights()?.relation_lane_weights().len())
    }
    ...
    pub fn common_alpha_factor(&self) -> Result<&[E], AkitaError> {
        Ok(self.weights()?.common_alpha_factor())
    }

    fn weights(&self) -> Result<&RelationWeightFactorization<E>, AkitaError> {
        match &self.prover.relation_state {
            RelationRoundState::QuotientFactored { weights, .. } => Ok(weights),
            RelationRoundState::ReducedDense { .. } => Err(AkitaError::InvalidSetup(
                "direct relation proof requires factored weights".into(),
            )),
        }
    }
    ...
    pub fn two_round_prefix_data(
        ...
        let Some(prefix) = self.prover.deferred_compact_prefix() else {
            return Ok(None);
        };
        let stage1_point = &prefix.stage1_point;
        ...
        Ok(Some(DirectRelationTwoRoundPrefixData {
            equality_first,
            equality_second,
            alpha: self.common_alpha_factor()?,
            lane_weights: self.weights()?.relation_lane_weights(),
            basis: self.prover.b,
            live_lane_count: self.prover.live_lane_count,
            coefficient_count: self.current_coefficient_count()?,
    ...
    pub fn bind_without_linear_terms(&mut self, challenge: E) -> Result<(), AkitaError> {
        let coefficient_round = self.prover.rounds_completed < self.prover.coefficient_bits();
        let RelationRoundState::QuotientFactored { weights, .. } = &mut self.prover.relation_state
        else {
            return Err(AkitaError::InvalidSetup(
                "direct relation binding requires factored weights".into(),
            ));
        };
        ...
        if coefficient_round {
            fold_evals_in_place(weights.components_mut().0, challenge);
        } else {
            fold_evals_in_place(weights.components_mut().1, challenge);
...
impl<E: Field + Ring + Fold + Unreduced + AkitaSerialize> RelationRangeImageProver<E> {
    pub fn prove_with_backend<F, T, B>(
        ...
        preparation: Option<B::Preparation>,
        transcript: &mut T,
        level: u32,
    ) -> Result<DirectRelationRangeProofOutput<E>, AkitaError>
    where
        ...
        B: DirectRelationRangeProofBackend<F, E>,
    {
        let Some(preparation) = preparation else {
            return prove_relation_range_cpu::<F, E, T>(self, transcript, level);
        };
