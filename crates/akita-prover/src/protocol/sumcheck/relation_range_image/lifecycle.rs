...
impl<E: FieldCore + FromPrimitiveInt + HasOptimizedFold + HasUnreducedOps>
    DirectRelationRangeProofState<E>
{
    ...
    pub fn compact_witness(&self) -> std::sync::Arc<[i8]> {
        match &self.prover.witness_state {
            WitnessState::CompactPrefix(witness) => std::sync::Arc::clone(witness),
            WitnessState::FoldedSuffix(_) => {
                unreachable!("direct Stage 2 state is created before witness folding")
            }
        }
    }
    ...
    pub fn relation_lane_weights(&self) -> &[E] {
        &self.prover.relation_lane_weights
    }
    ...
    pub fn bind(&mut self, challenge: E) {
        ...
        if self.prover.rounds_completed < self.prover.coefficient_bits() {
            ...
            self.prover.linear_terms.fold_coefficients(challenge);
        } else {
            fold_evals_in_place(&mut self.prover.relation_lane_weights, challenge);
            self.prover.linear_terms.fold_lanes(challenge);
...
impl<F, E> DirectRelationRangeProofBackend<F, E> for CpuBackend
where
    F: FieldCore + akita_field::CanonicalField,
    E: akita_field::ExtField<F>
        + FromPrimitiveInt
...
impl<F, E> DirectRelationRangeProofBackend<F, E> for OpeningCluster
where
    F: FieldCore + akita_field::CanonicalField,
    E: akita_field::ExtField<F>
        + FromPrimitiveInt
...
impl<E> RelationRangeImageProver<E>
where
    E: FieldCore + FromPrimitiveInt + HasUnreducedOps + HasOptimizedFold + AkitaSerialize,
{
    /// Route the complete Stage-2 proof through one operation backend.
