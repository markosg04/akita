    ...
    B: RuntimeRingSwitchProveBackend<F>,
{
    ...
    let prepare_relation_tail = || match (lp.ring_relation_mode, d_quotients) {
        (
            akita_types::RingRelationMode::QuotientLift,
            RelationDQuotientWitness::QuotientLift(d_quotients),
        ) => Ok(PreparedWitnessTail::QuotientLift(
        ...
        ) => Ok(PreparedWitnessTail::ReducedEvaluation),
    ...
    // Tail preparation and body emission have no data dependency.
    ...
    let (tail, out) = cfg_join!(prepare_relation_tail, emit_body);
    let tail = tail?;
    let mut out = out?;
    {
