...
pub(crate) trait MetalField: CanonicalField + FieldCore {
...
impl Fp128Limbs {
    ...
    pub(crate) fn into_field(self, index: usize) -> Result<F, MetalCommitError> {
        F::from_canonical_u128_checked(self.to_u128())
            .ok_or(MetalCommitError::NonCanonicalOutput { index })
    }
}
...
mod tests {
    ...
    fn canonical_limb_boundary_round_trips() {
        ...
        for value in values {
            let field = F::from_canonical_u128_checked(value).unwrap();
