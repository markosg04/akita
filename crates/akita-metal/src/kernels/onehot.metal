...
inline ulong akita_simd_shuffle_down_ulong(ulong value, ushort delta) {
    uint low = simd_shuffle_down((uint)value, delta);
    uint high = simd_shuffle_down((uint)(value >> 32u), delta);
    return (ulong)low | ((ulong)high << 32u);
}

inline ulong4 akita_simd_shuffle_down_ulong4(ulong4 value, ushort delta) {
    return ulong4(
        akita_simd_shuffle_down_ulong(value[0], delta),
        akita_simd_shuffle_down_ulong(value[1], delta),
        akita_simd_shuffle_down_ulong(value[2], delta),
        akita_simd_shuffle_down_ulong(value[3], delta));
}

kernel void akita_fp128_d512_fused_coefficient_packing_partials(
    device const uchar *lanes [[buffer(0)]],
    device const AkitaFp128 *combined_weights [[buffer(1)]],
    device AkitaFp128 *partials [[buffer(2)]],
    constant PackedCoefficientPackingIndexParams &index_params [[buffer(3)]],
    constant PackedOneHotCoefficientPackingParams &params [[buffer(4)]],
    uint thread_index [[thread_index_in_threadgroup]],
    uint3 threadgroup_index [[threadgroup_position_in_grid]])
{
    threadgroup uchar window_hots[D512_PACKING_TILES_PER_CHUNK * 256];
    threadgroup ushort window_records[D512_PACKING_TILES_PER_CHUNK * 256];
    threadgroup atomic_uint bucket_cursors[D512_PACKING_TILES_PER_CHUNK * 32];
    threadgroup ushort bucket_starts[D512_PACKING_TILES_PER_CHUNK * 32];
    ...
    ulong tile_end = min(
        tile_start + (ulong)D512_PACKING_TILES_PER_CHUNK,
        index_params.position_tiles);
    ulong tile_count = tile_end - tile_start;
    ulong window_positions = tile_count * 256ul;
    ulong cursor_count = tile_count * 32ul;

    for (ulong cursor = (ulong)thread_index;
         cursor < cursor_count;
         cursor += 256ul) {
        atomic_store_explicit(
            &bucket_cursors[cursor], 0u, memory_order_relaxed);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    for (ulong local_position = (ulong)thread_index;
         local_position < window_positions;
         local_position += 256ul) {
        ulong local_tile = local_position >> 8ul;
        ulong position = tile_start * 256ul + local_position;
        ulong ring = trace_block * index_params.num_positions + position;
        ulong row = ring * 2ul + parity;
        uint hot = 0u;
        if (position < index_params.num_positions && row < index_params.num_rows) {
            hot = (uint)lanes[row * index_params.lane_stride + column];
        }
        window_hots[local_position] = (uchar)hot;
        if (hot != 0u) {
            atomic_fetch_add_explicit(
                &bucket_cursors[local_tile * 32ul + (ulong)(hot >> 3u)],
                1u,
                memory_order_relaxed);
        }
    }
    ...
    if ((ulong)thread_index < tile_count) {
        ulong local_tile = (ulong)thread_index;
        uint running = 0u;
        for (uint local_bucket = 0u; local_bucket < 32u; ++local_bucket) {
            ulong cursor = local_tile * 32ul + (ulong)local_bucket;
            uint count = atomic_load_explicit(
                &bucket_cursors[cursor], memory_order_relaxed);
            bucket_starts[cursor] = (ushort)running;
            atomic_store_explicit(
                &bucket_cursors[cursor], running, memory_order_relaxed);
            running += count;
        }
    ...
    for (ulong local_position = (ulong)thread_index;
         local_position < window_positions;
         local_position += 256ul) {
        uint hot = (uint)window_hots[local_position];
        if (hot != 0u) {
            ulong local_tile = local_position >> 8ul;
            ulong cursor = local_tile * 32ul + (ulong)(hot >> 3u);
            uint destination = atomic_fetch_add_explicit(
                &bucket_cursors[cursor], 1u, memory_order_relaxed);
            window_records[local_tile * 256ul + (ulong)destination] =
                (ushort)(((hot & 7u) << 8u) | (uint)(local_position & 255ul));
        }
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    ulong4 sums = ulong4(0ul);
    for (ulong local_tile = 0ul; local_tile < tile_count; ++local_tile) {
        ulong cursor = local_tile * 32ul + (ulong)bucket;
        uint start = 0u;
        uint end = 0u;
        if (bucket_lane == 0u) {
            start = (uint)bucket_starts[cursor];
            end = atomic_load_explicit(
                &bucket_cursors[cursor], memory_order_relaxed);
        }
        start = simd_shuffle(start, bucket_owner);
        end = simd_shuffle(end, bucket_owner);
        ulong record_base = local_tile * 256ul;
        for (uint record_index = start + bucket_lane;
             record_index < end;
             record_index += 8u) {
            uint record = (uint)window_records[
                record_base + (ulong)record_index];
            ulong position = (tile_start + local_tile) * 256ul
                + (ulong)(record & 255u);
            ulong low = (ulong)(record >> 8u);
            AkitaFp128 weight = combined_weights[position * 8ul + low];
            sums += ulong4(
    ...
    ulong4 other = akita_simd_shuffle_down_ulong4(sums, 4u);
    if (bucket_lane < 4u) {
        sums += other;
    }
    other = akita_simd_shuffle_down_ulong4(sums, 2u);
    if (bucket_lane < 2u) {
        sums += other;
    }
    other = akita_simd_shuffle_down_ulong4(sums, 1u);
    if (bucket_lane == 0u) {
        ulong partial_index =
            (stream * 32ul + (ulong)bucket) * tile_chunks + tile_chunk;
        partials[partial_index] = akita_reduce_unsigned_limb_sums(sums + other);
    }
}
