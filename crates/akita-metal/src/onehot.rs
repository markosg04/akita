...
{
    #[tracing::instrument(skip_all, name = "MetalCommitBackend::onehot_commit_inner")]
    fn commit_inner_group(
        ...
        let metrics = MetalCommitMetrics {
            ...
            cpu_blocks: 0,
            metal_blocks: shape.num_blocks,
            metal_full_blocks: shape.num_blocks,
            ...
            metal_work_units: 0,
            cpu_rank_rows: 0,
            metal_rank_rows: plan.n_a,
            num_sources: sources.len(),
            hot_entries,
            lane_scan_ballots: 0,
            selected_lane_broadcasts: 0,
            field_additions: to_u64(field_additions, "field additions")?,
            reduction_field_additions: 0,
            gathered_matrix_bytes: to_u64(gathered_matrix_bytes, "gathered matrix bytes")?,
            modeled_matrix_read_bytes: to_u64(gathered_matrix_bytes, "modeled matrix read bytes")?,
            modeled_lane_read_bytes: 0,
            index_bytes: size_of_val(hot_indices.as_slice()),
            ...
            index_pack_time,
            opening_index_time: std::time::Duration::ZERO,
            opening_index_gpu_time: std::time::Duration::ZERO,
            opening_index_bytes: 0,
...
mod tests {
    ...
    use akita_types::{
        accumulate_matrix_field_elements_for_level, AkitaScheduleLookupKey, PolynomialGroupLayout,
        SetupMatrixCapacity,
    };
    ...
    fn full_commit_matches_cpu() {
        ...
        .unwrap();
        let metal_output = akita_prover::commit::<Cfg, OneHotPoly<F, u8>, MetalCommitBackend>(
            &polys,
            setup.expanded.as_ref(),
            &metal_stack,
            context,
